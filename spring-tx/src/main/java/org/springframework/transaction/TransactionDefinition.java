...
/**
 * 定义一个事务的完整属性的接口。基于类似 EJB CMT 属性的传播行为定义。
 *
 *
...
public interface TransactionDefinition {
	...
	/**
	 * 支持当前事务；如果不存在则以非事物方式执行。如果存在事务，则加入该事务。
	 *
	...
	/**
	 * 如果当前存在事务，则执行一个嵌套事务，否则就像 PROPAGATION_REQUIRED 一样。
	 *
	 ...
	 * 如果外部事务 commit, 嵌套事务也会被 commit, 这个规则同样适用于 roll back.
	 *
	...
	/**
	 * 默认隔离级别。MySQL 默认使用 REPEATABLE_READ 隔离级别
	 *
	 ...
	 * 隔离级别——读未提交
	 *
	 * 允许读取尚未提交的数据，可能会发生脏读、不可重复读、幻读。
	 * 这个级别允许被一个事务在提交之前（脏读）更改的行，被另一个事务所读取。
	 * 乳沟任何一个更改被回滚，那么第二个事务将会检索到无效的行。
	 *
	 ...
	 * 隔离级别——可重复读
	 * 防止脏读和不可重复读，可能会出现幻读。
	 * 这个级别禁止事务读取一个未提交的更改的行，它也禁止以下情况：一个事务读取一行，第二个事务更改了这行，
	 * 第一个事务重新读取改行，第二次获取不同的值（不可重复读）。
	 *
	 * 可重复读是这样的：
	 * 1. 第一个 session 开启一个事务，查询数据 1，第二 session 也开启一个事务，查询数据 1；
	 * 2. 然后 session1 修改数据 1，并且提交事务；
	 * 3. session2 继续查询数据 1，发现它查询的数据和第一次查询的一样，不是 session1 修改的数据；
	 * 4. session2 提及事务，然后再次查询，才能看到 session1 提交的修改数据。
	 *
	 *
